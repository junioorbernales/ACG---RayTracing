...
void PaintImage(Film* film)
{
    ...
    for (size_t lin = 0; lin < resY; lin++)
    {
        ...
        for (size_t col = 0; col < resX; col++)
        {
            double r = static_cast<double>(col) / static_cast<double>(resX);
            double g = static_cast<double>(lin) / static_cast<double>(resY);
            double b = 0.0;

            Vector3D output_color(r, g, b);
            film->setPixelValue(col, lin, output_color);
